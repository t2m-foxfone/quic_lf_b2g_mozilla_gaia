...
//          $('btChannel').classList.remove('disable');
    ...
    $('btTxRun').addEventListener('click', function() {
      ...
      //var runCommand = _self.getTxBTRunCommand(btTxPattern, btTxChannel,
      //                 btTxPacketType, btTxWhitening, btTxPower, btTxRxGain);
      var txRunCommand = '/system/bin/btTx_run '
                          + btTxChannel + ' ' + btTxPattern + ' '
    ...
    $('btVTA-input').addEventListener('change', function(evt) {
      ...
      dump('gaolu wifiSleep value  ' + value);
  ...
  getTxWifiRunCommand : function getTxWifiRunCommand(txpro, txfreq, txpwr, txrate) {
    return '/system/bin/wifitest tx cmd ' + txpro + ' '
           + txfreq + ' ' + txpwr + ' '
           + txrate + ' > /data/wifitext.txt';
  },
