...
var systemUpdate = {
  ...
  /*keep in ram.mozJrdFotaManager:get from gecko*/
  _mozJrdFota: null,
  ...
  init: function fs_init() {
    ...
    debug('init entry.');
    /*invoke getFotaManager once*/
    this._mozJrdFota = navigator.mozJrdFota;
  ...
  getNewPackage: function fs_getNewPackage() {
    ...
    if (this._mozJrdFota.JrdFotaActionStatus != 'NoAction') {
  ...
  remind: function fs_remind() {
    ...
    var actionStatus = this._mozJrdFota.JrdFotaActionStatus;
  ...
  sendReplyCommand: function fota_rply_cmd(category, value1, value2, value3) {
    ...
    if (self._isSend2Setting == false) {
      debug('system send to setting flag is false');
      return;
    }
  ...
  sendToSettingCommand: function fota_sendtosetting_command(category, value1) {
    var replyMsg;
    var self;
    ...
    /*setting exit ,no need to send msg*/
    if (self._isSend2Setting == false) {
      if (category === 'downloadFromSetting') {
        JrdFotaNotifiCation.handleOpenApp().bind(self);
      }
      return;
    }
    switch (category) {
      case 'downloadFromSetting':
        debug('send download from setting');
        replyMsg = {
          category: category,
          completeRate: value1
        };
